  ...
  <div class="flex-y-center flex-column full" v-if="isMobile()">
    ...
    <div class="flex-column mt-10 p-26 w-100 mobile" v-show="!loginMode">
      ...
      <div class="flex-y-center flex-column mt-30">
        <IscasButton :loading="nextLoading" style="width: 200px; height: 44px" @click="nextStep">{{
  ...
  <div v-else class="flex-y-center login-container full">
    ...
    <div class="core-div">
      <div class="title flex">
        <div :class="loginMode ? 'subTitle' : 'subTitle2'" @click="choseLogin(true)">
          <div class="font-18 fw-b" style="height: 61px; line-height: 62px">
            {{ $t('login.scan_code') }}
          </div>
          <div class="checkLine"></div>
        </div>
        <div :class="loginMode ? 'subTitle2' : 'subTitle'" @click="choseLogin(false)">
          <div class="font-18 fw-b" style="height: 61px; line-height: 62px">
      ...
      <div class="flex-xy-center flex-column" v-show="loginMode">
        <div class="mt-37" style="width: 240px; height: 240px" v-request="loading">
      ...
      <div class="flex-xy-center flex-column pc" v-show="!loginMode">
        <IscasInput
          ...
          v-model="subDomain"
        ></IscasInput>
        <div class="button-blue mt-130" style="width: 200px; height: 56px" @click="nextStep">
          {{ $t('buttons.common_next') }}
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
      ...
      loginUtils
        ...
        .then((result) => {
          ...
          if (result.code === 'GW-200') {
            ...
            this.errorMessage('login.space_not_found', 'login.space_not_found_mobile')
    ...
    gotoIpForSubDomain(data) {
      ...
      networkListener
        .ping(data.boxLanInfo.lanDomain, data.boxLanInfo.port, data.boxLanInfo.tlsPort)
        .then(() => {
        ...
        .catch(() => {
          networkListener
            .ping(data.boxLanInfo.lanIp, data.boxLanInfo.port, data.boxLanInfo.tlsPort)
            .then(() => {
              window.location.href =
                'http://' +
