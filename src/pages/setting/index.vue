  ...
  methods: {
    ...
    storapi() {
      getStorage().then((data) => {
        ...
        this.spaceSizeUnsed = dealSpace(data.spaceSizeTotal - data.spaceSizeUsed)
      })
    },
        ...
        if (result.code == 'ACC-200') {
          ...
          for (let member of result.results) {
            ...
            if (this.isTrial && member.aoId === this.user.aoId) {
              ...
              this.spaceSizeUnsed = dealSpace(storage.results.totalStorage - storage.results.userStorage)
            }
          }
